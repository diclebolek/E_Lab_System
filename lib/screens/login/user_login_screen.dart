  ...
  Future<void> _handleLogin() async {
    ...
    if (kIsWeb) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Web platformunda veritabanı bağlantısı desteklenmiyor. '
            'Lütfen mobil veya desktop uygulaması kullanın.',
          ),
    ...
    try {
      final result = await FirebaseService.signInWithTC(_tcController.text.trim(), _passwordController.text);
      ...
      if (mounted) {
        if (result != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TC kimlik veya şifre hatalı!')));
        }
      }
    } catch (e) {
      setState(() => _isLoading = false);
          ...
          SnackBar(
            content: Text(
              e.toString().contains('Unsupported')
                  ? 'Web platformunda veritabanı bağlantısı desteklenmiyor. Lütfen mobil veya desktop uygulaması kullanın.'
    ...
    if (mounted) {
      if (result != null) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Kayıt başarısız! Bu TC kimlik zaten kayıtlı olabilir.')));
      }
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(labelText: 'TC Kimlik Numarası', prefixIcon: Icon(Icons.badge)),
            ...
            InkWell(
              ...
              child: InputDecorator(
                ...
                child: Text(
                  ...
                  style: TextStyle(color: _selectedBirthDate != null ? Colors.black : Colors.grey.shade600),
                ),
              ),
            ),
            const SizedBox(height: 20),
          ...
          TextButton(
            ...
            child: Text(
              _isRegistering ? 'Zaten bir hesabınız var mı? Giriş yapın.' : 'Hesabınız yok mu? Kayıt olun.',
