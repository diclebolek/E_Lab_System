...
class PostgresService {
  ...
  static Future<Map<String, dynamic>?> signInWithTC(
    String tc,
    String password,
  ) async {
    try {
      ...
      // Geçici olarak düz metin karşılaştırması (hash'leme yapılmadığı için)
      if (passwordTrimmed != storedPassword) {
        return null;
      }
  ...
  static Future<bool> signInAsAdmin(String tc, String password) async {
    try {
      ...
      print('   Şifre uzunluğu: ${passwordTrimmed.length}');
      ...
      print('   Veritabanındaki şifre: "$storedPassword"');
      print('   Veritabanı şifre uzunluğu: ${storedPassword.length}');
      print('   Şifre eşleşmesi: ${passwordTrimmed == storedPassword}');
    ...
    try {
      ...
      if (tcNumber.isNotEmpty) {
        final userResult = await conn.execute(
          Sql.named(
            'SELECT id FROM users WHERE tc_number = @tc AND is_deleted = FALSE',
          ),
  ...
  static Future<Map<String, dynamic>?> getTahlilById(String id) async {
    try {
      ...
      final serumTypes = serumResult
          .map((r) => {'type': r[0], 'value': r[1]})
          .toList();
    ...
    try {
      ...
      if (updates.containsKey('serumTypes')) {
        ...
        for (final serum in serumTypes) {
          await conn.execute(
            Sql.named(
              'INSERT INTO serum_types (tahlil_id, type, value) VALUES (@tahlil_id, @type, @value)',
            ),
            parameters: {
              'tahlil_id': int.parse(id),
              'type': serum['type'] ?? '',
              'value': serum['value'] ?? '',
            },
          );
  ...
  static Future<bool> deleteTahlil(String id) async {
    try {
      ...
      // Tahlili sil
      await conn.execute(
        Sql.named('DELETE FROM tahliller WHERE id = @id'),
        parameters: {'id': int.parse(id)},
      );
  ...
  static Future<bool> updateGuide(
    ...
    try {
      ...
      if (newGuideName != null &&
          newGuideName.trim().isNotEmpty &&
          newGuideName != guideName) {
        await conn.execute(
          Sql.named(
      ...
      await conn.execute(
        Sql.named('DELETE FROM kilavuz_rows WHERE kilavuz_id = @id'),
  ...
  static Future<Map<String, dynamic>?> getGuide(String guideName) async {
    try {
      ...
      final guideResult = await conn.execute(
        Sql.named(
          'SELECT id, guide_name FROM kilavuzlar WHERE guide_name = @name',
        ),
        parameters: {'name': guideName},
      );
  ...
  static Future<Map<String, dynamic>?> getUserInfo() async {
    try {
      ...
      final result = await conn.execute(
        Sql.named(
          'SELECT id, tc_number, full_name, created_at, updated_at, is_deleted '
  ...
  static Future<bool> updateUserInfo({String? fullName}) async {
    try {
      if (_currentUserId == null) {
        return false;
      }
      ...
      await conn.execute(
        Sql.named(
          'UPDATE users SET full_name = @full_name, updated_at = CURRENT_TIMESTAMP '
          'WHERE id = @id',
        ),
        parameters: {'id': _currentUserId, 'full_name': fullName.trim()},
      );
  ...
  static Future<List<Map<String, dynamic>>> getAllTahliller() async {
    try {
      ...
      final result = await conn.execute(
        Sql.named(
          'SELECT id, full_name, tc_number, report_date, created_at '
          'FROM tahliller '
          'ORDER BY created_at DESC',
        ),
      );
  ...
  static Future<bool> updateAdminInfo({String? email, String? fullName, String? tcNumber}) async {
    try {
      ...
      if (email != null && email.trim().isNotEmpty) {
        ...
        if (checkResult.isNotEmpty) {
          throw Exception(
            'Bu e-posta adresi başka bir doktor tarafından kullanılıyor',
          );
        }
      ...
      if (tcNumber != null && tcNumber.trim().isNotEmpty) {
        ...
        if (checkResult.isNotEmpty) {
          throw Exception(
            'Bu TC kimlik numarası başka bir doktor tarafından kullanılıyor',
          );
        }
      ...
      final query =
          'UPDATE admins SET $updateFields, updated_at = CURRENT_TIMESTAMP WHERE id = @id';
