...
class PdfService {
  ...
  // Çıkarılan metinden bilgileri parse etme
  static Map<String, dynamic> parseTahlilData(String extractedText) {
    ...
    final namePatterns = [
      ...
      RegExp(
        r'([A-ZÇĞİÖŞÜ][a-zçğıöşü]+\s+[A-ZÇĞİÖŞÜ][a-zçğıöşü]+(?:\s+[A-ZÇĞİÖŞÜ][a-zçğıöşü]+)?)',
      ),
    ];
    ...
    final datePatterns = [
      ...
      RegExp(
        r'(?:Doğum|Doğum\s+Tarihi|Doğum\s+Tarih)[:\s]*(\d{1,2})[./-](\d{1,2})[./-](\d{4})',
        caseSensitive: false,
      ),
    ...
    // Cinsiyet - Daha esnek pattern'ler
    // Örnekler: "Cinsiyet: Erkek", "Cinsiyet: Kadın", "Erkek", "Kadın"
    final genderPatterns = [
      RegExp(
        r'(?:Cinsiyet|Cinsiyeti)[:\s]*(Erkek|Kadın|Kız|Bay|Bayan)',
        caseSensitive: false,
      ),
      RegExp(r'\b(Erkek|ERKEK|erkek|E|e|Bay|BAY|bay)\b'),
      RegExp(r'\b(Kadın|KADIN|kadın|K|k|Bayan|BAYAN|bayan|Kız|KIZ|kız)\b'),
    ];

    for (var pattern in genderPatterns) {
      final match = pattern.firstMatch(extractedText);
      if (match != null) {
        final genderText = match.group(1)?.toLowerCase() ?? '';
    ...
    // Hastalık Tanısı - PDF'den hastalık tanısını çıkar
    ...
    final sampleDatePatterns = [
      ...
      RegExp(
        r'(?:Alım\s+Tarihi|Alım\s+Tarih)[:\s]*(\d{1,2})[./-](\d{1,2})[./-](\d{4})',
        caseSensitive: false,
      ),
    ];
    ...
    for (var serumType in serumTypeOptions) {
      ...
      final patterns = [
        ...
        RegExp(
          '$serumType[\\s\\S]{0,50}?([\\d]+[.,]?\\d*)\\s*(?:mg/dl|mg|g/L|g/l|µg/ml|µg/mL)?',
          caseSensitive: false,
        ),
      ];
    ...
    // Rapor Tarihi - PDF'den çıkar
    final reportDatePatterns = [
      RegExp(
        r'(?:Rapor\s+Tarihi|Rapor\s+Tarih|Tarih)[:\s]*(\d{1,2})[./-](\d{1,2})[./-](\d{4})',
        caseSensitive: false,
      ),
    ];
