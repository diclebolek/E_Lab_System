...
class PatientTahlilHistoryScreen extends StatefulWidget {
  ...
  const PatientTahlilHistoryScreen({
    super.key,
...
class _PatientTahlilHistoryScreenState extends State<PatientTahlilHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.patientName} - Tahlil Geçmişi'),
                              ...
                              children: [
                                Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      // Hasta bilgileri
                                      _buildInfoRow('Adı Soyadı', tahlil.fullName),
                                      _buildInfoRow('T.C. Kimlik No', tahlil.tcNumber),
                                      _buildInfoRow('Yaş', _formatAge(tahlil)),
                                      _buildInfoRow('Cinsiyet', tahlil.gender),
                                      _buildEditableInfoRow('Hastalık Tanısı', tahlil.patientType, tahlil.id),
                                      _buildInfoRow('Numune Türü', tahlil.sampleType),
                                      const Divider(),
                                      // Serum değerleri
                                      const Text(
                                        'Serum Değerleri',
                                      ...
                                      ...tahlil.serumTypes.map((serum) {
                                        ...
                                        final previousValue = _getPreviousSerumValue(
                                          serum.type,
                                          index,
                                        );
                                        final changeIndicator = _getChangeIndicator(
                                          currentValue,
                                          previousValue,
                                        );
                                        // Kılavuza göre değerlendirme
                                        final guideEvaluation = _getSerumEvaluation(
                                          tahlil,
                                          serum.type,
                                          currentValue,
                                        );
                                        final showComparison = _selectedSerumType == null ||
                                            _selectedSerumType == serum.type;
                                        ...
                                        return Card(
                                          ...
                                          color: previousValue != null
                                              ? _getChangeColor(changeIndicator)
                                                  .withValues(alpha: 0.05)
                                              : (guideEvaluation != null
                                                  ? _getArrowColor(guideEvaluation['arrow'])
                                                      .withValues(alpha: 0.05)
                                                  : null),
                                          child: ListTile(
  ...
  Widget _buildEditableInfoRow(String label, String value, String tahlilId) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
