...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  // PDF yükleme ve hızlı değerlendirme alanlarını doldurma
  Future<void> _loadPdfForEvaluation() async {
    ...
    try {
      ...
      if (mounted) {
        final filledCount =
            parsedData.containsKey('birthDate') || parsedData.containsKey('age')
            ? 1
            : 0;
        final serumCount = parsedData['serumTypes'] != null
          ...
          SnackBar(
            content: Text(
              serumCount > 0 || filledCount > 0
                  ? 'PDF yüklendi! Doğum tarihi: ${filledCount > 0 ? "✓" : "✗"}, Serum değerleri: $serumCount adet'
                  : 'PDF yüklendi ancak bilgi bulunamadı. PDF formatını kontrol edin.',
            ),
    ...
    try {
      ...
      if (isPlus) {
        ...
        final minAge = int.tryParse(
          minAgeStr.replaceAll(RegExp(r'[^0-9]'), ''),
        );
        if (minAge != null) {
        ...
        final maxAge = maxAgeStr.isNotEmpty
            ? int.tryParse(maxAgeStr.replaceAll(RegExp(r'[^0-9]'), ''))
            : null;
  ...
  void _handleEvaluate() {
    if (_birthDateController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lütfen doğum tarihini girin!')),
      );
      return;
    }
    ...
    if (serumValues.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lütfen en az bir serum değeri girin!')),
      );
      return;
    }
    ...
    for (var guide in _guides) {
      ...
      for (var row in filteredRows) {
        ...
        if (serumValues.containsKey(serumType)) {
          ...
          if (arithMeanMin != null && arithMeanMax != null) {
            final arrow = _evaluateSerumValue(
              value,
              arithMeanMin,
              arithMeanMax,
            );
            evaluation['arithMean'] =
                '${row['arithMeanMin']}-${row['arithMeanMax']}';
          ...
          if (intervalMin != null && intervalMax != null) {
            ...
            evaluation['interval'] =
                '${row['intervalMin']}-${row['intervalMax']}';
      ...
      if (evaluations.isNotEmpty) {
        results.add({
          'guideName': guide['guideName'],
          'ageRange': filteredRows.isNotEmpty
              ? filteredRows[0]['ageRange']
              : '-',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Doktor Rapor Yönetim Paneli'),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                      ...
                      Flexible(
                        child: Container(
                          ...
                          child: IconButton(
                            ...
                            tooltip: 'PDF\'den Otomatik Doldur',
                          ),
                        ),
                      ),
                    ],
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'PDF yükleyerek doğum tarihi ve serum değerlerini otomatik doldurabilirsiniz',
                            style: TextStyle(
                              fontSize: 12,
                  ...
                  TextField(
                    key: ValueKey(
                      'birthDate_$_textFieldKey',
                    ), // UI güncellemesi için key
                  ...
                  Wrap(
                    ...
                    children: _serumTypes.map((type) {
                      final value = double.tryParse(
                        _serumControllers[type]!.text.trim(),
                      );
                      final evaluation = value != null && _age > 0
                          ? _getSerumEvaluation(type, value)
                          : null;
                      final statusColor = evaluation != null
                          ? _getArrowColor(evaluation['arrow'])
                          : Colors.grey;
                      final statusIcon = evaluation != null
                          ? _getStatusIcon(evaluation['arrow'])
                          : Icons.help_outline;
