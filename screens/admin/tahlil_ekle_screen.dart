...
class _TahlilEkleScreenState extends State<TahlilEkleScreen> {
  ...
  // PDF yükleme ve otomatik doldurma
  Future<void> _loadPdfAndFillForm() async {
    setState(() => _isLoadingPdf = true);

    try {
      // PDF dosyası seç
      final result = await PdfService.pickPdfFile();
      if (result == null) {
        setState(() => _isLoadingPdf = false);
        return;
      }

      // PDF bytes al (web uyumlu)
      final pdfBytes = await PdfService.getPdfBytes(result);
      if (pdfBytes == null) {
        if (mounted) {
      ...
      // PDF'den metin çıkar
      final extractedText = await PdfService.extractTextFromPdf(pdfBytes);
      if (extractedText.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('PDF\'den metin çıkarılamadı.')),
          );
        }
        setState(() => _isLoadingPdf = false);
        return;
      ...
      final parsedData = PdfService.parseTahlilData(extractedText);
      ...
      setState(() {
        ...
        if (parsedData['serumTypes'] != null) {
          final serumList =
              parsedData['serumTypes'] as List<Map<String, String>>;
          _serumTypes.clear();
          for (var serum in serumList) {
            _serumTypes.add(
              SerumType(type: serum['type'] ?? '', value: serum['value'] ?? ''),
            );
          }
        }
      });

      // PDF'den gelen verilerle otomatik kaydet
      if (mounted && _birthDate != null && _serumTypes.isNotEmpty && _fullNameController.text.trim().isNotEmpty) {
        ...
        if (mounted) {
          if (success) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('PDF yüklendi ve tahlil otomatik olarak kaydedildi!'),
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('PDF yüklendi ancak tahlil kaydedilirken bir hata oluştu. Lütfen manuel olarak kaydedin.'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('PDF başarıyla yüklendi ve form dolduruldu! Lütfen eksik bilgileri tamamlayıp kaydedin.'),
          ),
        );
      }
    } catch (e) {
  ...
  void _addSerumValue() {
    if (_selectedSerumType.isEmpty || _serumValueController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lütfen serum tipi ve değeri girin!')),
      );
      return;
    }
    ...
    setState(() {
      _serumTypes.add(
        SerumType(type: _selectedSerumType, value: _serumValueController.text),
      );
      _selectedSerumType = '';
  ...
  Future<void> _saveTahlil() async {
    ...
    if (_serumTypes.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lütfen en az bir serum değeri ekleyin!')),
      );
      return;
    }

    // Rapor tarihi - şimdiki zamanı kullan (PDF'den gelse bile form kaydedilirken güncel tarih kullanılır)
    final reportDate = DateFormat('dd/MM/yyyy HH:mm').format(DateTime.now());
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tahlil başarıyla kaydedildi!')),
      );
      _resetForm();
    } else if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tahlil kaydedilirken bir hata oluştu!')),
      );
    }
  }
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    // PDF Yükleme Butonu - Modern tasarım
                    Container(
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: const Color(0xFF0058A3).withValues(alpha: 0.2),
                        ),
                      ),
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const Text(
                              'PDF belgesi yükleyerek hasta ve tahlil bilgilerini otomatik olarak doldurabilirsiniz.',
                              style: TextStyle(
                                fontSize: 14,
                                color: Colors.grey,
                              ),
                    ...
                    TextFormField(
                      ...
                      onChanged: (value) =>
                          setState(() => _patientType = value),
                    ),
                    const SizedBox(height: 16),
                    TextFormField(
                      controller: _sampleTypeController,
                      decoration: const InputDecoration(
                        labelText: 'Numune Türü',
                    ...
                    DropdownButtonFormField<String>(
                      initialValue: _selectedSerumType.isEmpty
                          ? null
                          : _selectedSerumType,
                      decoration: const InputDecoration(
                        labelText: 'Serum Tipi Seçin',
                      ),
                      items: _serumTypeOptions.map((type) {
                        return DropdownMenuItem(value: type, child: Text(type));
                      }).toList(),
                      onChanged: (value) =>
                          setState(() => _selectedSerumType = value ?? ''),
                    ),
                    const SizedBox(height: 10),
                    TextFormField(
                      controller: _serumValueController,
                      decoration: const InputDecoration(
                        labelText: 'Serum Değeri',
                      ),
                      keyboardType: TextInputType.number,
                    ),
                    const SizedBox(height: 10),
                    ElevatedButton(
                      onPressed: _addSerumValue,
                      child: const Text('Serum Değeri Ekle'),
                    ),
                    if (_serumTypes.isNotEmpty) ...[
                      const SizedBox(height: 20),
                      const Text(
                        'Eklenen Serum Değerleri:',
